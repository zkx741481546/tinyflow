...
class Convolution1DForwardOp(Op):
    ...
    def gradient(self, node, output_grad):
        a = [0,0,0]
        return [convolution_1d_backward_op(node.inputs[0],node.inputs[1],output_grad, node.dataformat, node.padding, node.v,0,a),
                convolution_1d_backward_op(node.inputs[0],node.inputs[1],output_grad, node.dataformat, node.padding, node.v,1,a)]
    def infer_shape(self, node, input_shapes):
...
class Convolution1DBackwardOp(Op):
    def __call__(self, node_A, node_B, node_C, dataformat, padding, v,type, cache):
        new_node = Op.__call__(self)
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        ...
        ctx = ndarray.gpu(0)
        dinput = ndarray.empty(input_vals[0].shape,ctx)
        dfilter = ndarray.empty(input_vals[1].shape,ctx)
        if node.cache[0] == 0:
            gpu_op.convolution_1d_backward(input_vals[0],input_vals[2],input_vals[1],dfilter, dinput,node.dataformat,node.padding,node.v)
            node.cache[0] = 1
            node.cache[1] = dinput
            node.cache[2] = dfilter
...
class Convolution2DForwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        ...
        gpu_op.convolution_2d_forward(input_vals[0], input_vals[1], output_val, node.dataformat, node.padding,node.u, node.v)
    def gradient(self, node, output_grad):
        a = [0,0,0]
        return [convolution_2d_backward_op(node.inputs[0],node.inputs[1],output_grad, node.dataformat, node.padding, node.u, node.v,0,a),
                convolution_2d_backward_op(node.inputs[0],node.inputs[1],output_grad, node.dataformat, node.padding, node.u, node.v,1,a)]
    def infer_shape(self, node, input_shapes):

        return gpu_op.convolution_2d_forward_get_out_shape(input_shapes[0],input_shapes[1],node.dataformat,node.padding,node.u,node.v)

class Convolution2DBackwardOp(Op):
    def __call__(self, node_A, node_B, node_C, dataformat, padding,u, v,type, cache):
        new_node = Op.__call__(self)
        new_node.inputs = [node_A, node_B, node_C]
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        ...
        dfilter = ndarray.empty(input_vals[1].shape,ctx)
        if node.cache[0] == 0:
            gpu_op.convolution_2d_backward(input_vals[0],input_vals[2],input_vals[1],dfilter, dinput,node.dataformat,node.padding,node.u,node.v)
            node.cache[0] = 1
            node.cache[1] = dinput
            node.cache[2] = dfilter
...
class Convolution3DForwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        ...
        gpu_op.convolution_3d_forward(input_vals[0], input_vals[1], output_val, node.dataformat, node.padding,node.s1, node.s2,node.s3)
    def gradient(self, node, output_grad):
        a = [0,0,0]
        return [convolution_3d_backward_op(node.inputs[0],node.inputs[1],output_grad, node.dataformat, node.padding, node.s1, node.s2,node.s3,0,a),
                convolution_3d_backward_op(node.inputs[0],node.inputs[1],output_grad, node.dataformat, node.padding, node.s1, node.s2,node.s3,1,a)]
    def infer_shape(self, node, input_shapes):

        return gpu_op.convolution_3d_forward_get_out_shape(input_shapes[0],input_shapes[1],node.dataformat,node.padding,node.s1, node.s2,node.s3)

class Convolution3DBackwardOp(Op):
    def __call__(self, node_A, node_B, node_C, dataformat, padding,s1, s2, s3,type, cache):
        new_node = Op.__call__(self)
        new_node.inputs = [node_A, node_B, node_C]
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        ...
        dfilter = ndarray.empty(input_vals[1].shape,ctx)
        if node.cache[0] == 0:
            gpu_op.convolution_3d_backward(input_vals[0],input_vals[2],input_vals[1],dfilter, dinput,node.dataformat,node.padding,node.s1, node.s2,node.s3)
            node.cache[0] = 1
            node.cache[1] = dinput
            node.cache[2] = dfilter
...
class ActivationForwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        assert use_numpy==False
        gpu_op.activation_forward(input_vals[0],output_val,node.dataformat,node.activationMode)
        node.output[0] = output_val
    def gradient(self, node, output_grad):

        return [activation_backward_op(node.inputs[0],output_grad,node.output,node.dataformat,node.activationMode)]
    def infer_shape(self, node, input_shapes):
        return input_shapes[0]

class ActivationBackwardOp(Op):
    def __call__(self, node_A,node_B, output,dataformat,activationMode):
        new_node = Op.__call__(self)
        new_node.inputs = [node_A,node_B]
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        ...
        gpu_op.activation_backward(input_vals[0],output_val,node.output[0],input_vals[1],node.dataformat,node.activationMode)
    def gradient(self, node, output_grad):
        raise NotImplementedError
    def infer_shape(self, node, input_shapes):
        return input_shapes[0]
...
class Pooling1DForwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        assert use_numpy==False
        assert isinstance(input_vals[0], ndarray.NDArray)
        gpu_op.pooling_1d_forward(input_vals[0], output_val, node.dataformat, node.poolingMode, node.pad_w, node.v, node.filter_w)
        node.output[0] = output_val
    def gradient(self, node, output_grad):
        return [pooling_1d_backward_op(node.inputs[0],output_grad,node.output,node.dataformat, node.poolingMode, node.pad_w, node.v, node.filter_w)]
    def infer_shape(self, node, input_shapes):
        return gpu_op.pooling_1d_forward_get_out_shape(input_shapes[0],node.dataformat, node.poolingMode, node.pad_w, node.v, node.filter_w)

class Pooling1DBackwardOp(Op):
    def __call__(self, node_A,node_B, output,dataformat,poolingMode,pad_w,v,filter_w):
        new_node = Op.__call__(self)
        new_node.inputs = [node_A,node_B]
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        assert use_numpy==False
        assert isinstance(node.output[0], ndarray.NDArray)
        gpu_op.pooling_1d_backward(input_vals[0],node.output[0],input_vals[1],output_val,node.dataformat, node.poolingMode, node.pad_w, node.v, node.filter_w)
    def gradient(self, node, output_grad):
        raise NotImplementedError
    def infer_shape(self, node, input_shapes):
        return input_shapes[0]
...
class Pooling2DForwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        ...
        gpu_op.pooling_2d_forward(input_vals[0], output_val, node.dataformat, node.poolingMode, node.pad_h, node.pad_w, node.u, node.v, node.filter_h, node.filter_w)
        node.output[0] = output_val

    def gradient(self, node, output_grad):
        return [pooling_2d_backward_op(node.inputs[0],output_grad,node.output,node.dataformat, node.poolingMode, node.pad_h, node.pad_w, node.u, node.v, node.filter_h, node.filter_w)]
    def infer_shape(self, node, input_shapes):

        return gpu_op.pooling_2d_forward_get_out_shape(input_shapes[0],node.dataformat, node.poolingMode, node.pad_h, node.pad_w, node.u, node.v, node.filter_h, node.filter_w)

class Pooling2DBackwardOp(Op):
    def __call__(self, node_A,node_B, output,dataformat,poolingMode,pad_h, pad_w, u, v, filter_h, filter_w):
        new_node = Op.__call__(self)
        new_node.inputs = [node_A,node_B]
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        assert use_numpy==False
        assert isinstance(node.output[0], ndarray.NDArray)
        gpu_op.pooling_2d_backward(input_vals[0],node.output[0],input_vals[1],output_val,node.dataformat, node.poolingMode, node.pad_h, node.pad_w, node.u, node.v, node.filter_h, node.filter_w)
    def gradient(self, node, output_grad):
        raise NotImplementedError
    def infer_shape(self, node, input_shapes):
        return input_shapes[0]
...
class Pooling3DForwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        ...
        gpu_op.pooling_3d_forward(input_vals[0], output_val, node.dataformat, node.poolingMode, node.pad1, node.pad2,node.pad3, node.s1, node.s2,node.s3,node.filter1,node.filter2,node.filter3)
        node.output[0] = output_val
    def gradient(self, node, output_grad):
        return [pooling_3d_backward_op(node.inputs[0],output_grad,node.output,node.dataformat, node.poolingMode, node.pad1, node.pad2,node.pad3, node.s1, node.s2,node.s3,node.filter1,node.filter2,node.filter3)]
    def infer_shape(self, node, input_shapes):

        return gpu_op.pooling_3d_forward_get_out_shape(input_shapes[0],node.dataformat, node.poolingMode, node.pad1, node.pad2,node.pad3, node.s1, node.s2,node.s3,node.filter1,node.filter2,node.filter3)

class Pooling3DBackwardOp(Op):
    def __call__(self, node_A,node_B, output,dataformat,poolingMode,pad1, pad2,pad3, s1, s2,s3,filter1,filter2,filter3):
        new_node = Op.__call__(self)
        new_node.inputs = [node_A,node_B]
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        assert use_numpy==False
        assert isinstance(node.output[0], ndarray.NDArray)
        gpu_op.pooling_3d_backward(input_vals[0],node.output[0],input_vals[1],output_val,node.dataformat, node.poolingMode, node.pad1, node.pad2,node.pad3, node.s1, node.s2,node.s3,node.filter1,node.filter2,node.filter3)
    def gradient(self, node, output_grad):
        raise NotImplementedError
    def infer_shape(self, node, input_shapes):
        return input_shapes[0]
...
class DropoutForwardOp(Op):
    def __call__(self, node_A,dataformat,dropout,seed=0):
        new_node = Op.__call__(self)
        new_node.inputs = [node_A]
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        assert use_numpy==False
        assert isinstance(input_vals[0], ndarray.NDArray)
        node.reserveSpace_p[0] = gpu_op.dropout_forward(input_vals[0], output_val, node.dataformat, node.dropout, node.seed)
    def gradient(self, node, output_grad):
        return [dropout_backward_op(output_grad,node.reserveSpace_p,node.dataformat, node.dropout, node.seed)]
    def infer_shape(self, node, input_shapes):
        return input_shapes[0]

class DropoutBackwardOp(Op):
    def __call__(self, node_A,reserveSpace_p,dataformat,dropout,seed):
        new_node = Op.__call__(self)
        new_node.inputs = [node_A]
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        assert use_numpy==False
        gpu_op.dropout_backward(input_vals[0], output_val, node.dataformat, node.dropout, node.seed,node.reserveSpace_p[0])
    def gradient(self, node, output_grad):
        raise NotImplementedError
    def infer_shape(self, node, input_shapes):
        return input_shapes[0]
...
class FullyActivationForwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        ...
        gpu_op.activation_forward(inputs,output_val,node.dataformat,node.activationMode)
        node.output[0] = output_val
    def gradient(self, node, output_grad):
        return [fullyactivation_backward_op(node.inputs[0],output_grad,node.output,node.dataformat,node.activationMode)]
    def infer_shape(self, node, input_shapes):
        return input_shapes[0]
...
class FullyActivationBackwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy=False):
        ...
        gpu_op.activation_backward(inputs,output_val,node.output[0],input_vals[1],node.dataformat,node.activationMode)
    def gradient(self, node, output_grad):
        raise NotImplementedError
    def infer_shape(self, node, input_shapes):
        return input_shapes[0]
...
class L2lossOp(Op):
    ...
    def gradient(self, node, output_grad):
        # grad_A = (node.inputs[0] + -1 * node.inputs[1])*output_grad
        # grad_B = zeroslike_op(node.inputs[1])
        # return [grad_A, grad_B]
        a = [0, 0, 0]
