...
class Op(object):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
...
class Convolution1DForwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        ...
        memorytoSaving = gpu_op.convolution_1d_forward(input_vals[0], input_vals[1], output_val, node.cudnnlist[0], cudnnHandle)
        return memorytoSaving
    def gradient(self, node, output_grad):
...
class Convolution1DBackwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        ...
        if node.type == 0:
            memorytoSaving = gpu_op.convolution_backward_data(input_vals[0],input_vals[2],input_vals[1],output_val,node.cudnnlist[0], cudnnHandle)
        if node.type == 1:
            memorytoSaving = gpu_op.convolution_backward_filter(input_vals[0], input_vals[2], input_vals[1], output_val, node.cudnnlist[0], cudnnHandle)
        return memorytoSaving
    def gradient(self, node, output_grad):
        raise NotImplementedError
    def infer_shape(self, node, input_shapes, cudnnHandle):
...
class Convolution2DForwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        ...
        memorytoSaving = gpu_op.convolution_2d_forward(input_vals[0], input_vals[1], output_val, node.cudnnlist[0], cudnnHandle)
...
class Convolution3DForwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        ...
        memorytoSaving = gpu_op.convolution_3d_forward(input_vals[0], input_vals[1], output_val, node.cudnnlist[0], cudnnHandle)
        return memorytoSaving
    def gradient(self, node, output_grad):
...
class ActivationBackwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        ...
        gpu_op.activation_backward(input_vals[0],output_val,node.output[0],input_vals[1],node.activationMode, node.cudnnlist[0], cudnnHandle)
        return 0
    def gradient(self, node, output_grad):
        raise NotImplementedError
    def infer_shape(self, node, input_shapes, cudnnHandle):
...
class Pooling1DBackwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        assert use_numpy==False
        assert isinstance(node.output[0], ndarray.NDArray)
        gpu_op.pooling_1d_backward(input_vals[0],node.output[0],input_vals[1],output_val,node.cudnnlist[0], cudnnHandle)
        return 0
    def gradient(self, node, output_grad):
        raise NotImplementedError
    def infer_shape(self, node, input_shapes, cudnnHandle):
...
class Pooling2DBackwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        assert use_numpy==False
        assert isinstance(node.output[0], ndarray.NDArray)
        gpu_op.pooling_2d_backward(input_vals[0],node.output[0],input_vals[1],output_val,node.cudnnlist[0], cudnnHandle)
        return 0
    def gradient(self, node, output_grad):
        raise NotImplementedError
    def infer_shape(self, node, input_shapes, cudnnHandle):
...
class Pooling3DBackwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        assert use_numpy==False
        assert isinstance(node.output[0], ndarray.NDArray)
        gpu_op.pooling_3d_backward(input_vals[0],node.output[0],input_vals[1],output_val, node.cudnnlist[0], cudnnHandle)
        return 0
    def gradient(self, node, output_grad):
        raise NotImplementedError
    def infer_shape(self, node, input_shapes, cudnnHandle):
...
class DropoutForwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        ...
        node.reserveSpace_p[0],node.cudnnlist[0],memorytoSaving = gpu_op.dropout_forward(input_vals[0], output_val, node.dataformat, node.dropout, node.seed[0], node.inputd[0], cudnnHandle)
        return memorytoSaving
    def gradient(self, node, output_grad):
...
class FullyDropoutForwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        ...
        node.reserveSpace_p[0],node.cudnnlist[0], memorytoSaving = gpu_op.dropout_forward(input, output_val, node.dataformat, node.dropout, node.seed[0],node.inputd[0], cudnnHandle)
        return memorytoSaving
    def gradient(self, node, output_grad):
...
class FullyActivationBackwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        ...
        input = input_vals[0]
        # inputs = input.reshape((input.shape[0], 1, input.shape[1]))
        gpu_op.activation_backward(input,output_val,node.output[0],input_vals[1],node.activationMode,node.cudnnlist[0], cudnnHandle)
...
class BNForwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        ...
        memorytoSaving = gpu_op.bn_forward(input_vals[0], output_val,node.batchNormMode,node.n, node.Save_p[0],node.Save_p[1],node.cudnnlist[0], cudnnHandle)
        node.n = node.n + 1
...
class BNBackwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        ...
        memorytoSaving = gpu_op.bn_backward(input_vals[0],input_vals[1], output_val,node.batchNormMode, node.Save_p[0],node.Save_p[1],node.cudnnlist[0], cudnnHandle)
...
class FullyBNBackwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        ...
        input = input_vals[0]
        # inputs = input.reshape((input.shape[0], 1, input.shape[1]))
        memorytoSaving = gpu_op.bn_backward(input,input_vals[1], output_val,node.batchNormMode ,node.Save_p[0],node.Save_p[1],node.cudnnlist[0], cudnnHandle)
        return memorytoSaving
    def gradient(self, node, output_grad):
        raise NotImplementedError
    def infer_shape(self, node, input_shapes, cudnnHandle):
...
class ConcatBackwardOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        ...
        assert isinstance(input_vals[2], ndarray.NDArray)
        if node.type==0:
            gpu_op.concat_a_backward(input_vals[0], input_vals[1],input_vals[2],output_val)
        if node.type==1:
            gpu_op.concat_b_backward(input_vals[0],input_vals[1],input_vals[2], output_val)
    def gradient(self, node, output_grad):
        raise NotImplementedError
    def infer_shape(self, node, input_shapes, cudnnHandle):
        if node.type == 0:
...
class SgdOp(Op):
    ...
    def compute(self, node, input_vals, output_val, cudnnHandle, cublasHandle, use_numpy=False):
        assert use_numpy==False
        gpu_op.sgd_update(input_vals[0],input_vals[1],node.learning_rate)
        return 0
    def gradient(self, node, output_grad):
        raise NotImplementedError
    def infer_shape(self, node, input_shapes, cudnnHandle):
