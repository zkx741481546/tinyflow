...
# from pycode.tinyflow import gpu_op, train, ndarray, TrainExecute, TrainExecute_Adam_vDNNall
import numpy as np
...
def run(num_step, type, batch_size, gpu_num, file_name):
    ...
    record = record_GPU.record("DenseNet", type, gpu_num, file_name)
    record.start()

    print("DenseNet" + " type" + str(type) + " start")

    dense_net = DenseNet121(num_step, type, batch_size, gpu_num, file_name)
    dense_net.dense_net(num_step=num_step, n_class=1000, X_val=X_val, y_val=y_val)

    print("DenseNet" + " type" + str(type) + " finish")
