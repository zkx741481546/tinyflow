...
class Inceptionv4(threading.Thread):
    def __init__(self, num_step, type, batch_size, gpu_num, file_name):
        ...
        self.f1 = open('./log/' + 'type' + str(type) + '_InceptionV4_' + file_name + '_record_1.txt', 'w')
        self.f3 = open('./log/' + 'type' + str(type) + '_InceptionV4_' + file_name + '_record_3.txt', 'w')
        self.f6 = open('./log/' + 'type' + str(type) + '_InceptionV4_' + file_name + '_record_6.txt', 'w')
        self.f7 = open('./log/' + 'type' + str(type) + '_InceptionV4_' + file_name + '_record_7.txt', 'w')
        self.type = type
        if type == 0:
    ...
    # stem
        ...
        return 0
    def run(self):
         record = record_GPU.record("InceptionV4", self.type, self.gpu_num, self.file_name)
         record.start()
         print("InceptionV4" + " type" + str(self.type) + " start")
         inception_v4 = Inceptionv4(self.num_step, self.type, self.batch_size, self.gpu_num, self.file_name)
